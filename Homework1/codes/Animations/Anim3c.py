...
def normal(point, scale=False):
    x, y = point[:2]
    vec = np.array([x/(np.hypot(x, y))**3, y/(np.hypot(x, y))**3, 0])
    mag = np.linalg.norm(vec)
...
class Anim3c(Scene):
    def construct(self):
        func_tex = (Tex(r"$\vec{\bf{v}} = \dfrac{\vec{\bf{r}}}{r^2}$").to_edge(UL).add_background_rectangle(BLACK))
        ...
        stream = StreamLines(streamlines, x_range=[-7.5, 7.5, 0.2], y_range=[-7.5, 7.5, 0.2], padding=1, stroke_width=2)
